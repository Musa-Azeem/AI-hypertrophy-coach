        ...
        def sample_data():
            ...
            if len(window) != 256:
                ...
                with torch.no_grad():
                    ...
                    keyAveragesTable = prof.key_averages().table(sort_by="cpu_time_total")
                    print(keyAveragesTable)
                    lines = keyAveragesTable.strip().split('\n')
                    last_line = lines[-1]
                    start_index = last_line.find(":") + 2
                    end_index = last_line.find("ms")
                    inference_time = float(last_line[start_index:end_index])
                    #print(f'Extracted Inference Time: {inference_time:.3f}')

                    inference_times.append(inference_time)
                    input_windows_lens.append(len(input_windows))
                    
                    print(inference_times)
                    print(input_windows_lens)
