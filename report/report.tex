...
In this work we explore the novel task of predicting RiR in real time using wearable, easy-to-apply sensors. As a first step torward this goal, we present a novel neural network-based pipeline for detecting ``near-failure'' repetitions in real time using a wrist-mounted inertial measurement unit (IMU). Our contributions are as follows:
...
\begin{itemize}
    ...
    \item We develop a classification model to identify near-failure repetitions, defined as repetitions performed with two or fewer remaining repetitions in reserve (RiR $\leq$ 2). 
